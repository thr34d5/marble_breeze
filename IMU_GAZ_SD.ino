...
int a = 0;
const int A_MQ4_pin = A0;
const int A_MQ811_pin = A1;
const int A_MQ7_pin = A2;
const int DOUTpin = 8;
int limit;
...
void loop() {
  ...
  if (millis() - displayTimer > 250) {
    ...
    myFile = SD.open("data.txt", FILE_WRITE);    //open file on SD card
    if (myFile) {
      myFile.print("!ANG:");
      myFile.print(rollInDegrees);
      myFile.print(",");
      myFile.print(pitchInDegrees);
      myFile.print(",");
      myFile.println(yawInDegrees);
